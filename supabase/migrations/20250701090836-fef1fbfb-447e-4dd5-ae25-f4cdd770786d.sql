...
-- Update the admin analytics view to properly handle referral scenarios
-- Ensure original admin's response time is preserved when tickets are referred
...
original_admin_response_times AS (
  -- Calculate response times for admins who first catered tickets (before any referrals)
  SELECT 
    ...
    AVG(
      CASE 
        ...
        ELSE NULL
      END
    ) as avg_original_response_time_hours
  FROM tickets t
  JOIN (
    -- Get the very first assignment activity for each ticket
...
referral_response_times AS (
  ...
  -- This is for admins who received referrals
  SELECT 
...
combined_response_times AS (
  -- Combine original and referral response times
  SELECT 
    COALESCE(oart.admin_id, rrt.admin_id) as admin_id,
    CASE 
      -- If admin has both original and referral response times, average them
      WHEN oart.avg_original_response_time_hours IS NOT NULL AND rrt.avg_referral_response_time_hours IS NOT NULL
      THEN (oart.avg_original_response_time_hours + rrt.avg_referral_response_time_hours) / 2
      -- If admin only has original response time (they catered tickets first)
      WHEN oart.avg_original_response_time_hours IS NOT NULL
      THEN oart.avg_original_response_time_hours
      -- If admin only has referral response time (they only received referrals)
  ...
  FROM original_admin_response_times oart
  FULL OUTER JOIN referral_response_times rrt ON oart.admin_id = rrt.admin_id
),
resolution_times AS (
...
SELECT 
  ...
  -- Use the combined response time calculations that preserve original admin's response time
...
-- Update the summary stats function to use the corrected data
...
BEGIN
  ...
  SELECT 
    ...
    -- Calculate weighted average response time to include all admins' contributions
    ROUND(
      CASE 
        WHEN SUM(CASE WHEN aav.avg_response_time_hours > 0 THEN aav.total_tickets_catered ELSE 0 END) > 0
        THEN SUM(aav.avg_response_time_hours * aav.total_tickets_catered) / 
             SUM(CASE WHEN aav.avg_response_time_hours > 0 THEN aav.total_tickets_catered ELSE 0 END)
        ELSE 0
      END, 6
    ) as avg_response_time_hours,
    ROUND(
      CASE 
        WHEN SUM(CASE WHEN aav.avg_resolution_time_hours > 0 THEN aav.total_tickets_catered ELSE 0 END) > 0
        THEN SUM(aav.avg_resolution_time_hours * aav.total_tickets_catered) / 
             SUM(CASE WHEN aav.avg_resolution_time_hours > 0 THEN aav.total_tickets_catered ELSE 0 END)
        ELSE 0
      END, 6
    ) as avg_resolution_time_hours
