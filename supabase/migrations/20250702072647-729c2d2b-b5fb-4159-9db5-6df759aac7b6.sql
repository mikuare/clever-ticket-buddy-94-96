...
-- Fix admin analytics to remove duplicate counts and implement real-time referral behavior
...
WITH admin_ticket_counts AS (
  ...
  SELECT 
    ...
    -- Total tickets catered and handled until resolution (excluding ANY referred tickets)
    COUNT(CASE 
...
admin_referral_counts AS (
  ...
  SELECT 
    p.id as admin_id,
    -- Tickets escalated/referred to other admins (including pending for real-time visibility)
    COUNT(DISTINCT CASE 
...
SELECT 
  ...
  ROUND(COALESCE(art.avg_resolution_time_hours, 0), 6) as avg_resolution_time_hours
  
FROM admin_ticket_counts atc
LEFT JOIN admin_referral_counts arc ON atc.admin_id = arc.admin_id
LEFT JOIN combined_response_times crt ON atc.admin_id = crt.admin_id
LEFT JOIN admin_resolution_times art ON atc.admin_id = art.admin_id
ORDER BY atc.admin_name;

-- Update the date-filtered function with the same fixed logic
...
BEGIN
  ...
  WITH admin_ticket_counts AS (
    ...
    SELECT 
      ...
      -- Total tickets catered and handled until resolution within date range (excluding ANY referred tickets)
      COUNT(CASE 
        WHEN t.assigned_admin_id = p.id 
...
-- Also create a missing date-filtered summary stats function
CREATE OR REPLACE FUNCTION get_admin_summary_stats_by_date_range(start_date date DEFAULT NULL::date, end_date date DEFAULT NULL::date)
RETURNS TABLE(
