...
-- Update admin analytics view to remove department column and improve calculations
DROP VIEW IF EXISTS admin_analytics_view;
...
WITH admin_ticket_stats AS (
  SELECT 
    ...
    -- Average response time (from ticket submission to catering/assignment) in hours
    AVG(
      CASE 
    ...
    -- Average resolution time (from catering/assignment to resolution) in hours
    AVG(
      CASE 
...
-- Update the function to get overall admin summary statistics
