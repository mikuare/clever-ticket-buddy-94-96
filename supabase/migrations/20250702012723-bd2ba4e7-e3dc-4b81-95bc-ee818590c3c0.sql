...
-- Update the admin analytics view to support date filtering by accepting date parameters
-- We'll create a new function that can filter by date range
CREATE OR REPLACE FUNCTION get_admin_analytics_by_date_range(
...
RETURNS TABLE(
  ...
  admin_name TEXT,
  email TEXT,
  total_tickets_catered BIGINT,
  tickets_in_progress BIGINT,
  tickets_resolved BIGINT,
  tickets_escalated BIGINT,
...
BEGIN
  ...
  WITH admin_ticket_stats AS (
    SELECT 
      ...
      -- Total tickets catered (assigned and accepted referrals) within date range
      COUNT(CASE 
      ...
      -- Tickets in progress within date range
      COUNT(CASE 
        WHEN t.assigned_admin_id = p.id 
      ...
      -- Tickets resolved within date range
      COUNT(CASE 
        WHEN t.assigned_admin_id = p.id 
  ...
  -- Response times for tickets within date range
  filtered_response_times AS (
    SELECT 
    ...
    FROM (
      -- First cater response times within date range
      SELECT 
    ...
    FULL OUTER JOIN (
      -- Referral acceptance response times within date range
      SELECT 
    ...
    ) rart ON fcrt.admin_id = rart.admin_id
  ),
  -- Resolution times within date range
  ...
  SELECT 
    ...
    ROUND(COALESCE(frt.avg_response_time_hours, 0), 6) as avg_response_time_hours,
    ROUND(COALESCE(frest.avg_resolution_time_hours, 0), 6) as avg_resolution_time_hours
  FROM admin_ticket_stats ats
  LEFT JOIN filtered_response_times frt ON ats.admin_id = frt.admin_id
  LEFT JOIN filtered_resolution_times frest ON ats.admin_id = frest.admin_id
  ORDER BY ats.admin_name;
END;
$$;

-- Create a function to get summary stats by date range
CREATE OR REPLACE FUNCTION get_admin_summary_stats_by_date_range(
  start_date DATE DEFAULT NULL,
  end_date DATE DEFAULT NULL
)
RETURNS TABLE(
