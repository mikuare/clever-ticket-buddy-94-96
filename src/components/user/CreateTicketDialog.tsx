...
const CreateTicketDialog = ({ classificationCooldowns, canCreateTicketForClassification, onTicketCreated, triggerButton }: CreateTicketDialogProps) => {
  ...
  const getCooldownMessage = () => {
    ...
    if (selectedClassificationOnCooldown) {
      return `Please wait ${cooldownMinutes} ${cooldownMinutes === 1 ? 'minute' : 'minutes'} before submitting another "${formData.classification}" ticket. You can select a different classification to submit immediately.`;
    }
          ...
          {formData.classification && selectedClassificationOnCooldown && (
            <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-md">
               <p className="text-sm text-yellow-800">
                 ⏱️ You recently submitted a "{formData.classification}" ticket. Please wait {cooldownMinutes} {cooldownMinutes === 1 ? 'minute' : 'minutes'} before submitting another ticket of this type, or select a different classification to proceed immediately.
               </p>
            </div>
          )}
