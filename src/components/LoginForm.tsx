...
const LoginForm = ({
  ...
  const { teamMembers: digitalizationTeam, loading: digitalizationLoading } = useDigitalizationTeam();
  const { teamMembers: itTeam, loading: itLoading } = useITTeam();
        ...
        <div className="bg-muted/30 p-4 border-t border-border">
          <div className="space-y-6">
            ...
            <div>
              ...
              {digitalizationLoading ? (
                <div className="flex justify-center">
                  <div className="w-6 h-6 border-2 border-primary/30 border-t-primary rounded-full animate-spin"></div>
                </div>
              ) : digitalizationTeam.length > 0 ? (
                <div className="overflow-x-auto pb-2">
                  <div className="flex gap-3 min-w-max px-2">
                    {digitalizationTeam.map((member) => (
                      <Card key={member.id} className="flex-shrink-0 w-24 bg-card/50 border border-border/50">
                ...
                <p className="text-center text-muted-foreground text-sm">No team members available</p>
              )}
            </div>
